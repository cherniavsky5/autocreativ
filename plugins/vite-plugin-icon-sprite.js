...
export default function IconSpritePlugin() {
  async function generateIconSprite() {
    ...
    const iconsDir = path.join(process.cwd(), 'src', 'assets', 'img', 'icons');
    ...
    await fs.writeFile(path.join(process.cwd(), 'src', 'assets', 'img', 'icon-sprite.svg'), sprite);
  }
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.watcher.add(path.join(process.cwd(), 'src', 'assets', 'img', 'icons', '*.svg'));
